...
return new class extends Migration
{
    ...
    public function down()
    {
        Schema::table('discounts', function (Blueprint $table) {
            // Note: This rollback assumes we want to go back to the original schema
            // Add original columns if necessary
            $table->unsignedBigInteger('product_id')->nullable();
            $table->decimal('discount', 8, 2)->nullable();

            // Drop new columns
            $table->dropColumn(['name', 'persentase', 'description', 'created_at', 'updated_at']);
        });
    }
};
